...
class Dispute extends ActiveRecord {
    ...
    public function rules() {
        return array(
            array('ref, status, added, added_by', 'required'),
            array('status', 'numerical', 'integerOnly' => true),
            array('ref', 'length', 'max' => 255),
            array('added_by, closed_by', 'length', 'max' => 11),
            array('closed', 'safe'),
            array('id, ref, status, added, added_by, closed, closed_by', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function beforeValidate() {
        ...
        else if($this->status > 0) {    //  && empty($this->closed)
